...
export default function AuthShell({ title, subtitle, children, sideTitle, sideText }) {
  return (
    <div className={`${ui.container} py-10 text-slate-900 dark:text-white`}>
      <div className="grid lg:grid-cols-2 gap-5 items-stretch">
        {/* Left: form */}
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none p-6 md:p-8">
          <h2 className="text-2xl font-semibold">{title}</h2>
          <p className="text-slate-600 dark:text-white/70 text-sm mt-1">{subtitle}</p>
          <div className="mt-6">{children}</div>
        </div>

        {/* Right: marketing panel */}
        <div className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-white/10 bg-linear-to-b from-white/10 to-transparent p-8 hidden lg:block">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-slate-600 dark:text-white/70">{sideText}</p>
