...
export default function ReputationInsights({ userId, onNavigate }) {
    ...
    return (
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl">
        <div className="flex items-center justify-center py-12">
          ...
          <p className="text-white/60">ARIS initializing intelligence analysis…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start gap-6 relative z-10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h2 className="text-2xl font-bold text-white tracking-tight">Reputation Intelligence</h2>
            ...
            <p className="text-white/70 text-sm leading-relaxed max-w-2xl">{intelligence.summary}</p>
          ...
          <div className="text-right flex-shrink-0 bg-slate-950/50 p-4 rounded-2xl border border-slate-800 min-w-[120px]">
            <div className="text-4xl font-black text-white mb-1 font-mono">{data.score}</div>
      ...
      {/* ── Tab Bar ── */}
      <div className="flex gap-2 border-b border-slate-800 overflow-x-auto pb-2 scrollbar-hide">
        {TABS.map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="min-h-[350px]">
        ...
        {activeTab === 'strengths' && (
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-3">
                  {intelligence.positives.length > 0 ? (
                    intelligence.positives.map((pos, i) => (
                      <div key={i} className="flex gap-3 items-center text-sm text-white/70 bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                        {pos}
                      </div>
                    ))
                  ) : (
                    <p className="text-slate-500 text-sm italic">Begin more sessions to identify strength patterns.</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={m.label} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-center hover:scale-105 transition-transform">
            ...
            {stats.subScores && stats.totalRatings > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <h3 className="text-white font-bold mb-4 text-sm uppercase tracking-widest">Sub-score breakdown</h3>
                  ...
                  {[
                    ...
                    <div key={f.key} className="flex flex-col gap-2">
                      <div className="flex justify-between text-xs text-slate-400">
                        <span>{f.label}</span>
                        <span className="font-mono font-bold text-white">{stats.subScores[f.key]}/5</span>
                      </div>
        ...
        {activeTab === 'weaknesses' && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="grid gap-3">
              {intelligence.negatives.length > 0 ? (
                intelligence.negatives.map((neg, i) => (
                  <div key={i} className="flex gap-3 items-center text-sm text-white/70 bg-red-500/5 p-4 rounded-xl border border-red-500/10">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-white/60 font-semibold tracking-wide uppercase text-xs">No active concerns identified</p>
                </div>
              )}
            </div>
            ...
            {stats.raterDiversity !== null && stats.raterDiversity !== undefined && (
              <div className="mt-6 p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Rater diversity</span>
            ...
            {stats.completionRate !== null && stats.completionRate !== undefined && (
              <div className="mt-4 p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Session completion rate</span>
        ...
        {activeTab === 'roadmap' && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <div className="bg-purple-500/5 border border-purple-500/20 rounded-2xl p-6 text-white/80 leading-relaxed whitespace-pre-line">
              {intelligence.roadmap}
            </div>
            <div className="mt-4 p-4 bg-slate-800/30 rounded-xl border border-slate-700 text-xs text-slate-400">
              Targeting <span className="text-white font-bold">{getTierLabel(data.nextTierThreshold)}</span> tier — {data.pointsToNextTier} points needed.
            </div>
          </div>
        )}
            ...
            <div className="space-y-3">
              {intelligence.suggestions.length > 0 ? (
                intelligence.suggestions.map((sug, i) => (
                  <div
                    key={i}
                    className="bg-slate-800/40 p-8 rounded-3xl border border-white/5 group hover:border-indigo-500/30 transition-all"
                  >
                    <div className="flex gap-6 items-start mb-6">
                      <div className="w-12 h-12 rounded-2xl bg-slate-950 flex items-center justify-center text-lg font-black text-indigo-400 border border-slate-800 group-hover:bg-indigo-500 group-hover:text-white transition-all shadow-xl shadow-black/20">
                        {i + 1}
                      </div>
                      <div className="flex-1">
                        <h4 className="text-white font-bold text-xl mb-4 tracking-tight">{sug.text}</h4>
                        ...
                        {sug.targetTab && (
                          <button
                            ...
                            className="flex items-center gap-3 bg-indigo-500 hover:bg-indigo-400 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-95 shadow-lg shadow-indigo-500/20"
                          >
        ...
        {activeTab === 'forecast' && (
          <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-2 gap-8 items-center bg-slate-950 p-8 rounded-3xl border border-slate-800">
                <div className="space-y-1">
                  <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Current momentum</div>
                ...
                <div className="space-y-1">
                  <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">7-day projection</div>
            ...
            {benchmark ? (
              <>
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
                  ...
                  <h3 className="text-xl font-bold text-white mb-2">Peer ranking</h3>
                  <p className="text-slate-400 text-sm mb-6">How you compare to the full user base.</p>
                  <div className="grid grid-cols-3 gap-6 mb-8">
                    <div className="bg-slate-950 rounded-2xl p-6 border border-slate-800">
                      <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Percentile</div>
                      <div className="text-3xl font-black text-blue-400 font-mono">{benchmark.percentile}<span className="text-lg">th</span></div>
                    </div>
                    <div className="bg-slate-950 rounded-2xl p-6 border border-slate-800">
                      <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Platform avg</div>
                      <div className="text-3xl font-black text-white font-mono">{benchmark.avgCohortScore}</div>
                    </div>
                    <div className="bg-slate-950 rounded-2xl p-6 border border-slate-800">
                      <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Users ahead</div>
                      <div className="text-3xl font-black text-white font-mono">{benchmark.usersAhead}</div>
                    </div>
                  </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center">
                ...
                <p className="text-slate-400">Benchmark data unavailable — complete more sessions to unlock peer comparisons.</p>
              </div>
            )}
          </div>
        )}
            ...
            {scoreHistory && scoreHistory.length > 0 ? (
              <div className="space-y-3 max-h-[450px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                {scoreHistory.map((entry, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 bg-slate-800/40 rounded-xl border border-slate-700">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-white/80">{entry.reason || 'Score updated'}</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center">
            {intelligence.risks.length > 0 ? (
              <>
                ...
                <h3 className="text-xl font-bold text-white mb-2">Integrity warnings</h3>
              ...
              <>
                ...
                <h3 className="text-xl font-bold text-white mb-2">No high-risk factors</h3>
        ...
        <div className="flex justify-between items-center px-2">
          ...
          <div className="flex gap-2">
            {stats.badges.map((b, i) => (
              <div
                key={i}
                className="w-6 h-6 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-[10px] text-indigo-400"
                title={b}
              >
                <FiTarget />
              </div>
