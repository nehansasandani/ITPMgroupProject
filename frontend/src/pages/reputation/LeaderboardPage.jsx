    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 selection:bg-indigo-500/30">
      ...
      >
        ...
        <div className="max-w-6xl mx-auto relative z-10 text-center">
          ...
          <p className="text-slate-700 dark:text-white/40 dark:text-white/50 dark:text-white/60 max-w-2xl mx-auto text-lg font-medium leading-relaxed mb-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-300">
      ...
      <div className="max-w-6xl mx-auto px-6 pb-32">
        ...
        {/* ── FILTER & SEARCH HUB ── */}
        <div className="bg-white dark:bg-slate-900 shadow-sm dark:shadow-none/40 border border-slate-200 dark:border-slate-200 dark:border-white/10 backdrop-blur-xl rounded-[2.5rem] p-4 mb-10 shadow-2xl shadow-indigo-500/5">
          <div className="flex flex-wrap items-center gap-4">
            
            {/* Period Toggle */}
            <div className="flex p-1.5 bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none rounded-2xl border border-slate-200 dark:border-slate-200 dark:border-white/10">
              {PERIODS.map(p => (
                <button
                  key={p.id}
                  ...
                  className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all duration-300 ${
                    period === p.id ? "bg-indigo-500 text-slate-900 dark:text-white shadow-xl shadow-indigo-500/20" : "text-slate-700 dark:text-white/40 dark:text-white/50 hover:text-slate-300"
                  }`}
                >
                  {p.icon} {p.label}
                </button>
            ...
            <div className="flex-1 min-w-[200px] relative">
              <FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700 dark:text-white/40 dark:text-white/50" />
              <input
                type="text"
                placeholder="Find talent by name or ID..."
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none/50 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl text-sm outline-none focus:border-indigo-500/50 transition-all font-medium"
              />
            </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3.5 rounded-2xl border font-bold text-xs uppercase tracking-widest transition-all ${
                filtersOpen || activeFilterCount > 0 ? "bg-indigo-500 text-slate-900 dark:text-white border-indigo-400" : "bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none border-slate-200 dark:border-slate-200 dark:border-white/10 text-slate-700 dark:text-white/40 dark:text-white/50 dark:text-white/60 hover:text-slate-900 dark:text-white"
              }`}
            >
          ...
          {filtersOpen && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-200 dark:border-slate-200 dark:border-white/10 animate-in slide-in-from-top-4 duration-300">
        ...
        {/* ── THE MASTER LIST ── */}
        <div className="bg-white dark:bg-slate-900 shadow-sm dark:shadow-none/20 rounded-[3rem] border border-slate-200 dark:border-slate-200 dark:border-white/10 overflow-hidden backdrop-blur-sm">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-700 dark:text-white/40 dark:text-white/50">
          ...
          {displayed.length === 0 && (
            <div className="py-20 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-700 dark:text-white/40 dark:text-white/50 dark:text-white/60 mb-1">No contributors found</h3>
              <p className="text-sm text-slate-700 dark:text-white/40">Try adjusting your skill or period filters.</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className={`relative group ${isMain ? "scale-110 z-10" : "scale-100 opacity-90"} transition-all duration-500`}>
      <div className={`absolute -top-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 transition-all group-hover:-translate-y-2`}>
        ...
        <span className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-white/40 dark:text-white/50">{rankMeta[rank].label}</span>
      </div>

      <div className={`pt-10 pb-8 px-8 rounded-[3rem] bg-white dark:bg-slate-900 shadow-sm dark:shadow-none/60 border ${isMain ? "border-indigo-500/30" : "border-slate-200 dark:border-slate-200 dark:border-white/10"} backdrop-blur-2xl text-center group-hover:bg-white dark:bg-slate-900 shadow-sm dark:shadow-none transition-all`}>
        <div className="w-24 h-24 mx-auto rounded-full bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none border-2 border-slate-200 dark:border-slate-200 dark:border-white/10 overflow-hidden mb-6 group-hover:scale-105 transition-all shadow-2xl shadow-black/40">
        ...
        <h3 className="text-xl font-black tracking-tight text-slate-900 dark:text-white mb-1">{user.userId?.fullName || "Contributor"}</h3>
        <p className="text-[10px] uppercase font-bold text-slate-700 dark:text-white/40 dark:text-white/50 tracking-widest mb-4">{user.userId?.department || "Unassigned"}</p>
        ...
        <div className="flex justify-between items-end gap-2 mb-8">
           <div className="text-left">
              <span className="block text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-white/40 dark:text-white/50 mb-0.5">Global Rep</span>
              <span className="text-3xl font-black text-slate-900 dark:text-white font-mono">{user.score}</span>
           </div>
           <div className="text-right">
              <span className="block text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-white/40 dark:text-white/50 mb-0.5">Badges</span>
        ...
        <button 
          onClick={onView}
          className="w-full py-4 rounded-2xl bg-indigo-500 hover:bg-indigo-400 text-slate-900 dark:text-white text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
        >
          View Performance
        </button>
      </div>
    ...
    <tr className="group hover:bg-white/5 transition-all cursor-pointer" onClick={onView}>
      ...
      <td className="py-8 px-4">
        <div className="flex items-center gap-5">
           <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none border border-slate-200 dark:border-slate-200 dark:border-white/10 overflow-hidden group-hover:scale-110 transition-all">
           ...
           <div>
              <div className="text-base font-black text-slate-900 dark:text-white tracking-tight">{entry.userId?.fullName}</div>
              <div className="text-[10px] font-bold text-slate-700 dark:text-white/40 dark:text-white/50 uppercase tracking-widest">{entry.userId?.studentId}</div>
           </div>
        </div>
      </td>
      ...
      <td className="py-8 px-4 hidden md:table-cell">
        <div className="flex gap-1.5">
           {entry.badges?.slice(0, 2).map((b, i) => (
             <span key={i} className="px-2 py-1 bg-white/5 rounded-md text-[9px] text-slate-700 dark:text-white/40 dark:text-white/50 dark:text-white/60 uppercase font-bold">{b}</span>
           ))}
           {entry.badges?.length > 2 && <span className="text-[9px] text-slate-700 dark:text-white/40">+{entry.badges.length - 2}</span>}
        </div>
      </td>
      <td className="py-8 px-4 text-right">
        <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{entry.score}</span>
      </td>
      <td className="py-8 px-10 text-right">
        <button className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none border border-slate-200 dark:border-slate-200 dark:border-white/10 text-slate-700 dark:text-white/40 dark:text-white/50 hover:text-indigo-400 group-hover:border-indigo-500/30 transition-all">
          <FiMoreHorizontal />
        </button>
      </td>
    </tr>
...
function AdvancedSelect({ label, options, value, onChange }) {
  return (
    <div className="space-y-2">
      <label className="text-[10px] font-bold text-slate-700 dark:text-white/40 uppercase tracking-widest px-2">{label}</label>
      <div className="relative">
        <select 
          value={value}
          onChange={e => onChange(e.target.value)}
          className="w-full appearance-none bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-3 text-sm text-slate-300 outline-none focus:border-indigo-500/50 transition-all"
        >
          {options.map(o => <option key={o} value={o}>{o}</option>)}
        </select>
        <FiChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-700 dark:text-white/40 dark:text-white/50 pointer-events-none" />
      </div>
    </div>
  );
}
...
function PerformanceCardModal({ userObj, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-white/80 dark:bg-[#020617]/90 backdrop-blur-md backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 shadow-sm dark:shadow-none border border-slate-200 dark:border-white/10 w-full max-w-4xl rounded-[3rem] overflow-hidden shadow-2xl relative">
        <button onClick={onClose} className="absolute top-8 right-8 p-3 rounded-2xl bg-white/5 text-slate-700 dark:text-white/40 dark:text-white/50 dark:text-white/60 hover:text-slate-900 dark:text-white transition-all z-10">
          <FiX size={20} />
        </button>
        ...
        {loading ? (
          <div className="py-40 text-center flex flex-col items-center">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 dark:text-white/40 dark:text-white/50 animate-pulse">Syncing Perf Metrics</p>
          </div>
        ) : (
          <div className="flex flex-col md:flex-row h-full">
            
            {/* Sidebar Stats */}
            <div className="w-full md:w-80 bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none p-10 border-r border-slate-200 dark:border-slate-200 dark:border-white/10 flex flex-col items-center text-center">
               <div className="w-32 h-32 rounded-full border-4 border-indigo-500/20 p-1 mb-6 relative group">
                  <div className="w-full h-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden shadow-2xl">
                     {userObj.profilePic ? (
                  ...
                  <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-xl bg-indigo-500 flex items-center justify-center text-slate-900 dark:text-white shadow-xl">
                     <FiAward />
                  </div>
               </div>
               
               <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white mb-1">{userObj.fullName}</h2>
               <p className="text-[10px] font-bold text-slate-700 dark:text-white/40 dark:text-white/50 uppercase tracking-widest leading-loose mb-2">
            ...
            <div className="flex-1 p-10 overflow-y-auto max-h-[70vh] scrollbar-hide">
              <div className="flex items-center justify-between mb-8">
                 ...
                 <div className="text-[10px] font-bold text-slate-700 dark:text-white/40 dark:text-white/50 uppercase tracking-widest">Global Index</div>
              </div>

              <div className="grid gap-4">
                {rep?.categoryScores?.length > 0 ? (
                  rep.categoryScores.map((cs, i) => (
                    <div key={i} className="p-6 bg-white/5 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-3xl group hover:border-indigo-500/30 transition-all">
                       <div className="flex justify-between items-start mb-6">
                          <div>
                             ...
                             <div className="text-[9px] font-bold text-slate-700 dark:text-white/40 dark:text-white/50 uppercase tracking-widest">Metric Analysis</div>
                          </div>
                  ...
                  <div className="py-20 text-center bg-white/5 rounded-[2rem] border border-dashed border-slate-200 dark:border-white/10">
                    <FiNavigation size={32} className="mx-auto mb-4 text-slate-700" />
                    <p className="text-xs font-bold text-slate-700 dark:text-white/40 dark:text-white/50 uppercase tracking-[0.2em]">No Verified Metrics Yet</p>
                  </div>
                )}
              </div>
            </div>
...
function MiniStat({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between p-4 bg-white/5 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-2xl w-full">
       <div className="flex items-center gap-3">
          {icon}
          <span className="text-[9px] font-black uppercase tracking-widest text-slate-700 dark:text-white/40 dark:text-white/50">{label}</span>
       </div>
       <span className="text-sm font-black text-slate-900 dark:text-white font-mono">{value}</span>
    </div>
  );
}
...
function SmallBar({ label, value, color }) {
  ...
  return (
    <div>
       <div className="flex justify-between text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-white/40 dark:text-white/50 mb-2">
          <span>{label}</span>
          <span className="text-slate-300 font-mono">{value.toFixed(1)}</span>
       </div>
       <div className="h-1.5 bg-slate-50 dark:bg-slate-950 shadow-sm dark:shadow-none rounded-full overflow-hidden border border-slate-200 dark:border-slate-200 dark:border-white/10">
