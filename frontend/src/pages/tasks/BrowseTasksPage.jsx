...
function TaskAcceptModal({ task, busy, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-lg rounded-3xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-950 p-6 text-slate-900 dark:text-white shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Description */}
        <div className="mt-3">
          <div className="text-xs text-slate-500 dark:text-white/50 uppercase tracking-wide mb-1">Description</div>
          <p className="text-sm text-slate-700 dark:text-white/80 leading-relaxed">{task.description}</p>
        </div>

        {/* Expected outcome */}
        <div className="mt-3">
          <div className="text-xs text-slate-500 dark:text-white/50 uppercase tracking-wide mb-1">Expected Outcome</div>
          <p className="text-sm text-slate-700 dark:text-white/80">{task.expectedOutcome}</p>
        </div>

        {/* Seeker (poster) details */}
        <div className="mt-4 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none p-4">
          <div className="text-xs text-slate-500 dark:text-white/50 uppercase tracking-wide mb-2">Task Seeker</div>
          <div className="text-sm text-slate-700 dark:text-white/90 font-medium">
            {task.createdBy?.fullName || "Unknown"}
          </div>
          {task.createdBy?.studentId && (
            <div className="text-xs text-slate-500 dark:text-white/60 mt-0.5">{task.createdBy.studentId}</div>
          )}
          {task.createdBy?.email && (
            <div className="text-xs text-slate-500 dark:text-white/60 mt-0.5">{task.createdBy.email}</div>
          )}
        </div>

        {/* Time & venue */}
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none p-3">
            <div className="text-xs text-slate-500 dark:text-white/50 uppercase tracking-wide mb-1">Session Time</div>
            <div className="text-sm text-slate-700 dark:text-white/90">{task.duration} minutes</div>
          </div>
          <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none p-3">
            <div className="text-xs text-slate-500 dark:text-white/50 uppercase tracking-wide mb-1">Mode</div>
            <div className="text-sm text-slate-700 dark:text-white/90">{task.mode}</div>
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-white/60">
...
function SuccessToast({ message, onClose }) {
  ...
  return (
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-100 px-6 py-3 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 text-emerald-100 text-sm shadow-2xl backdrop-blur flex items-center gap-3">
      <span>✅ {message}</span>
      <button onClick={onClose} className="text-emerald-300 hover:text-slate-900 dark:hover:text-white text-xs ml-2">
        ✕
      </button>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 text-slate-900 dark:text-white">
      ...
      {errorMsg && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-100 px-6 py-3 rounded-2xl border border-red-400/20 bg-red-400/10 text-red-100 text-sm shadow-2xl backdrop-blur flex items-center gap-3">
          ...
          <button onClick={() => setErrorMsg("")} className="text-red-300 hover:text-slate-900 dark:hover:text-white text-xs ml-2">
            ✕
          </button>
        </div>
      )}
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-slate-600 dark:text-white/70">No open tasks found.</div>
        ) : (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    ...
                    <div className="shrink-0">
                      {isMyOwnTask ? (
                        <button
                          disabled
                          className="px-4 py-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 shadow-sm dark:shadow-none text-sm text-white/40 cursor-not-allowed"
                        >
                          Your Task
                        </button>
                      ) : (
