...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    const task = await Task.create({
      ...
      venue: mode === "Meet" ? (venue || "").trim().slice(0, 100) : "",
      createdBy: req.user.id,
    });
...
router.patch("/:id/accept", requireAuth, async (req, res) => {
  try {
    ...
    // Create session record for this match, carrying the venue
...
router.patch("/:id/complete", requireAuth, async (req, res) => {
  try {
    ...
    await Session.findOneAndUpdate(
      { task: task._id, status: "ACTIVE" },
      { status: "COMPLETED", endedAt: new Date() }
    );

    // Reward helper: increment completedTasksCount and reputation
    if (task.acceptedBy) {
      const helperId = task.acceptedBy._id || task.acceptedBy;
      const helper = await User.findById(helperId);
      if (helper) {
        helper.completedTasksCount = (helper.completedTasksCount || 0) + 1;
        helper.reputation = Math.min(5, parseFloat(((helper.reputation || 0) + 0.2).toFixed(2)));
...
router.patch("/:id", requireAuth, async (req, res) => {
  try {
    ...
    // Validate venue for in-person meetings
    if (mode === "Meet" && (!venue || venue.trim().length < 3)) {
      return res.status(400).json({ message: "Venue is required for in-person meetings (min 3 characters)" });
    }
    ...
    task.venue = mode === "Meet" ? (venue || "").trim().slice(0, 100) : "";
